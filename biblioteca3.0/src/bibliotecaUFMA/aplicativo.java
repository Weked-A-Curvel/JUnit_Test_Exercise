...
public class aplicativo extends JFrame{
  ...
  public static aplicativo telaPrincipal = new aplicativo();
  public static BorderLayout border = new BorderLayout();
  public static biblioteca Biblioteca = new biblioteca();
  public static DataControlClass DataControl = new DataControlClass();
  
  //menu e barra de status que faltam

  public static void main(String [] args) throws IOException{
    aplicativo.carregaDados();	//primeiro carrega as informacoes de biblioteca	
    //DataControl.carregaDados(Biblioteca);
	  
	telaPrincipal.setTitle("M.S.B - Mini Sistema de Biblioteca");
    //telaPrincipal.setDefaultCloseOperation(EXIT_ON_CLOSE);
	telaPrincipal.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
	///faz com que a janela nao seja fechada no X, e abre a oportunidade de como definir este execucao, portando é ai que esta sendo chamada a funcao de salvar
	telaPrincipal.addWindowListener(new WindowAdapter(){
		public void windowClosing(WindowEvent evento){
			try{
				int resp = JOptionPane.showConfirmDialog(null, "Deseja Finalizar?", "Finalizar", JOptionPane.YES_NO_OPTION);
    ...
    painel = new login(Biblioteca);
    telaPrincipal.add(painel);
    telaPrincipal.setResizable(false);
    telaPrincipal.setVisible(true);
    telaPrincipal.setSize(500,520);
    telaPrincipal.setLocation(450, 100);
    
  }
  
  public static boolean /*void*/ carregaDados() throws FileNotFoundException{
/*	File arquivoFuncionarios;
	...
	*/
	Biblioteca = new DataControlClass().carregaDados(Biblioteca);  
	  
	return true ;
  }
  
  public static boolean/*void*/ SalvaDados() throws FileNotFoundException, IOException{
	 if(!new DataControlClass().salvaDados(Biblioteca)) {
		 return false;
	 }
	  /*File arquivoFuncionarios;
	 File arquivoUsuarios;
